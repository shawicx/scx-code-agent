...
def sec_expert_node(state: SharedReviewState) -> Dict[str, List[AgentIssue]]:
    """安全专家节点：审查代码安全缺陷（并发处理）"""
    # 初始化 LLM 客户端
    client = LLMClient()

    # 加载 prompt
    role_prompt = load_prompt("security.md")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            ...
            if file_info.get("content") is not None  # 跳过删除文件
        }
